...
# Load Configuration
from gan_human_en_configs import (CONFIG_GAN_HUMAN_EN,device)
...
train_dataloader = DataLoader(data, batch_size=CONFIG_GAN_HUMAN_EN['batch_size'], shuffle=True, pin_memory=True)  

# Load Model
model = GAN(CONFIG_GAN_HUMAN_EN, device)
# Train Model
model.train(train_dataloader, val_RG, CONFIG_GAN_HUMAN_EN['latent_dim'], CONFIG_GAN_HUMAN_EN['epochs'],initial_temp=CONFIG_GAN_HUMAN_EN['init_temp'],final_temp=CONFIG_GAN_HUMAN_EN['final_temp'],step=CONFIG_GAN_HUMAN_EN['step'])