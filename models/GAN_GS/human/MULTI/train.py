...
# Load Configuration
from gan_human_multi_configs import (CONFIG_GAN_HUMAN_MULTI,device)
...
train_dataloader = DataLoader(data, batch_size = CONFIG_GAN_HUMAN_MULTI['batch_size'], shuffle=True, pin_memory=True)    

# Load Model
model = GAN(CONFIG_GAN_HUMAN_MULTI, device)
# Train Model
model.train(train_dataloader, val_RG, CONFIG_GAN_HUMAN_MULTI['latent_dim'], CONFIG_GAN_HUMAN_MULTI['epochs'],initial_temp = CONFIG_GAN_HUMAN_MULTI['init_temp'], final_temp = CONFIG_GAN_HUMAN_MULTI['final_temp'], step = CONFIG_GAN_HUMAN_MULTI['step'])