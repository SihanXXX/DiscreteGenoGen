...
class ResGenerator(nn.Module):
    ...
    def forward(self, z: torch.tensor, temperature = 1.0, hard = False):
        ...
        Parameters:
            ...
            hard (bool): The hard parameter in Gumbel-Softmax determines whether the output is a soft probability distribution (hard=False during training) or a one-hot discrete vector (hard=True during inference)
        Returns:
...
class GAN(object):
    ...
    def generate_fake(self, nb, temperature=1.0, batch_size=2000):
        """
        ...
        for start_idx in range(0, nb, batch_size):
            ...
            with torch.no_grad():
                AG = trained_model(noise, temperature=temperature, hard=True)
    ...
    def train(self, 
        ...
        Parameters:
            ...
            val_data (pd.DataFrame): validation geno dataframe.
        ...
        for epoch in range(epochs):
            ...
            if (epoch+1)%step == 0:
                ...
                metrics_result = train_evaluator(val_data, fake_geno, ["precision_recall","fixation_index","pca","geno_freq"], "real", "syn", metric_dir+"/")
