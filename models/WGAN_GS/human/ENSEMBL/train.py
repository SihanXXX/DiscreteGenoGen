...
# Load Configuration
from wgan_human_en_configs import (CONFIG_WGAN_HUMAN_EN,device)
...
# Dataloader
data = GenotypeDataset(data_path, CONFIG_WGAN_HUMAN_EN['snp_repr'], CONFIG_WGAN_HUMAN_EN['rescale_height'])
train_dataloader = DataLoader(data, batch_size=CONFIG_WGAN_HUMAN_EN['batch_size'], shuffle=True, pin_memory=True)   

# Load Model
model = WGAN_GP(CONFIG_WGAN_HUMAN_EN, device)

# Train Model
model.train(train_dataloader, val_RG, val_sex.to(device), val_height.to(device), CONFIG_WGAN_HUMAN_EN['latent_dim'], CONFIG_WGAN_HUMAN_EN['epochs'],initial_temp=CONFIG_WGAN_HUMAN_EN['init_temp'],final_temp=CONFIG_WGAN_HUMAN_EN['final_temp'],step=CONFIG_WGAN_HUMAN_EN['step'])