...
class ResGenerator(nn.Module):
    ...
    def __init__(self, 
        ...
        self.input_dim = self.latent_dim + self.sex_dim + self.height_dim
        # Layers params
        ...
        for i in range(len(self.hidden_dims) - 1):
            self.res_blocks.append(nn.Sequential(
                ResNetBlockGenerator(self.hidden_dims[i] + self.sex_dim + self.height_dim, self.negative_slope),
                nn.Linear(self.hidden_dims[i]+self.sex_dim+self.height_dim, self.hidden_dims[i+1]),  # Transition layer
    ...
    def forward(self, z: torch.tensor, sex: torch.tensor, height: torch.tensor, temperature = 1.0, hard = False):
        ...
        """
        x = torch.cat((z, sex, height), 1)
        x = self.initial(x)
        for block in self.res_blocks:
            x = torch.cat((x, sex, height), 1)
            x = block(x)
        x = self.final(x)
...
class ResCritic(nn.Module):
    ...
    def __init__(self, x_dim: int,
        ...
        # Initial Layer
        self.initial = nn.Sequential(nn.Linear(self.x_dim *3+self.sex_dim+self.height_dim, self.hidden_dims[0]), nn.LeakyReLU(self.negative_slope))

        # ResBlock Layer
        self.res_blocks = nn.ModuleList()
        for i in range(len(self.hidden_dims) - 1):
            self.res_blocks.append(nn.Sequential(
                ResNetBlockCritic(self.hidden_dims[i]+self.sex_dim+self.height_dim, self.negative_slope),
                nn.Linear(self.hidden_dims[i]+self.sex_dim+self.height_dim, self.hidden_dims[i+1]),  # Transition layer
                nn.LeakyReLU(self.negative_slope) 
            ))
...
class WGAN_GP(object):
    ...
    def generate_fake(self, nb, sex, height, temperature=1.0):
        """
        ...
        with torch.no_grad():  # Disable gradient calculations
            AG = trained_model(noise, sex, height, temperature=temperature, hard=True)
    ...
    def train(self, 
        ...
        for epoch in range(epochs):
            ...
            if (epoch+1)%step == 0:
                ...
                metrics_result = train_evaluator(val_data_geno_df, fake_geno, ["precision_recall","fixation_index","pca","allele_freq","geno_freq"], "real", "syn", metric_dir+"/")
