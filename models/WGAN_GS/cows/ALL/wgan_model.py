...
class ResGenerator(nn.Module):
    ...
    def __init__(self, 
        ...
        """Parameters:
            ...
            pheno_dim (int): dimension of pheno condition
        ...
        self.input_dim = self.latent_dim + self.pheno_dim * 4 
        # Layers params
        ...
        for i in range(len(self.hidden_dims) - 1):
            self.res_blocks.append(nn.Sequential(
                ResNetBlockGenerator(self.hidden_dims[i]+self.pheno_dim*4, self.negative_slope),
                nn.Linear(self.hidden_dims[i]+self.pheno_dim*4, self.hidden_dims[i+1]),  # Transition layer
                nn.LeakyReLU(self.negative_slope) 
            ))

        #final layer
        self.final = nn.Linear(self.hidden_dims[-1]+self.pheno_dim*4, 3*self.output_dim)
            

    def forward(self, z: torch.tensor, pheno: torch.tensor, temperature = 1.0, hard = False):
        """ Main function to generate from input noise vector.
        ----
        Parameters:
            x (torch.tensor): input noise vector
            pheno (torch.tensor): torch tensor of dimension 1
        ...
        """
        x = torch.cat((pheno, pheno, z, pheno, pheno), 1)
        x = self.initial(x)
        for block in self.res_blocks:
            x = torch.cat((pheno, pheno, x, pheno, pheno), 1)
            x = block(x)
        x = torch.cat((pheno, pheno, x, pheno, pheno), 1)
        x = self.final(x)
...
class ResCritic(nn.Module):
    ...
    def __init__(self, x_dim: int,
        ...
        # Initial Layer
        self.initial = nn.Sequential(nn.Linear(self.x_dim *3 + 4 * self.pheno_dim, self.hidden_dims[0]), nn.LeakyReLU(self.negative_slope))

        # ResBlock Layer
        self.res_blocks = nn.ModuleList()
        for i in range(len(self.hidden_dims) - 1):
            self.res_blocks.append(nn.Sequential(
                ResNetBlockCritic(self.hidden_dims[i]+ 4 * self.pheno_dim, self.negative_slope),
                nn.Linear(self.hidden_dims[i]+ 4 * self.pheno_dim, self.hidden_dims[i+1]),  # Transition layer
                nn.LeakyReLU(self.negative_slope) 
            ))
    ...
    def forward(self, x, pheno: torch.tensor):
...
class WGAN_GP(object):
    ...
    def __init__(self, config: dict, device: torch.device):
        ...
        self.pheno_dim = config['pheno_dim']
    ...
    def gradient_penalty(
        ...
        Parameters:
            ...
            pheno (torch.tensor): pheno tensor
        Returns:
        ...
        d_interpolates = self.D(interpolates, pheno)
    ...
    def generate_fake(self, nb, pheno, temperature=1.0, batch_size=1500):
        """
        Generate fake SNP sequences encoded as 0, 1, 2 using a trained generator in small batches.
        ...
        Parameters:
            ...
            pheno (torch.tensor): Phenotype condition tensors
        Return:
            converted_AG: a numpy array containing the generated SNP sequence encoded as 0, 1, 2.
        """
        ...
        for start_idx in range(0, nb, batch_size):
            ...
            current_pheno = pheno[start_idx:end_idx]

            # Disable gradient calculations for memory efficiency
            with torch.no_grad():
                AG = trained_model(noise, current_pheno, temperature=temperature, hard=True)
    ...
    def train(self, 
        ...
        Parameters:
            ...
            val_data_pheno_tensor (torch.tensor): validation pheno torch tensor. 
        ...
        for epoch in range(epochs):
            ...
            # Load batches
            for geno, pheno in TrainDataLoader:
                # Batch losses
                ...
                for _ in range(self.n_critic) : 
                    ...
                    gen_outputs = self.G(batch_z, pheno, temperature=current_temperature)

                    # Forward pass for discriminator on real samples 
                    crit_real = self.D(geno, pheno)
                
                    # Forward pass for discriminator on generated samples
                    crit_fake = self.D(gen_outputs.detach(), pheno)
                
                    # Compute gradient penalty term
                    gp = self.gradient_penalty(geno, gen_outputs, pheno)
                ...
                # Generator forward pass
                gen_outputs = self.G(batch_z, pheno, temperature=current_temperature)
                crit_fake = self.D(gen_outputs, pheno)
                # Generator loss
            ...
            print(f"Epoch [{epoch+1}/{epochs}], Critic Loss: {epoch_crit:.6f}, Generator Loss: {epoch_gen:.6f}, Gradient Penalty: {epoch_gp:.6f}")
            # Write loss values to the file after each epoch
            with open(self.log_file, "a") as f:
                f.write(f"{epoch+1}\t{epoch_crit:.6f}\t{epoch_gen:.6f}\t{epoch_gp:.6f}\n")
            ...
            if (epoch+1)%step == 0:
                ...
                fake_geno = pd.DataFrame(self.generate_fake(val_data_geno_df.shape[0], val_data_pheno_tensor, temperature=current_temperature), columns=val_data_geno_df.columns, dtype=float)
                metrics_result = train_evaluator(val_data_geno_df, fake_geno, ["precision_recall","fixation_index","pca","allele_freq","geno_freq"], "real", "syn", metric_dir+"/", k=100)
