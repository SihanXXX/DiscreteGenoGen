...
class NoisePredictor(nn.Module):
    def __init__(self, data_dim, time_emb_dim, sex_emb_dim, height_emb_dim,
                 hidden_dim_1, hidden_dim_2, hidden_dim_3, total_timesteps: int = 1000):
        """
        Args:
            ...
            sex_emb_dim (int): Dimension for projecting the binary sex variable.
            height_emb_dim (int): Dimension for projecting the continuous height variable.
        ...
        self.sex_emb_dim = sex_emb_dim
        self.height_emb_dim = height_emb_dim
        self.total_timesteps = total_timesteps
        
        # For sex, use an embedding layer for the two categories (0 for female, 1 for male).
        self.sex_emb = nn.Embedding(2, sex_emb_dim)
        # For height, use a linear projection from 1 to height_emb_dim.
        self.height_proj = nn.Linear(1, height_emb_dim)
        
        # The overall input dimension is: data_dim (noisy data) + time_emb_dim + sex_emb_dim + height_emb_dim.
        self.input_dim = data_dim + time_emb_dim + sex_emb_dim + height_emb_dim
        ...
        # Second layer: before this layer we inject time, sex, and height embeddings again.
        # Its input dimension becomes hidden_dim_1 + (time_emb_dim + sex_emb_dim + height_emb_dim).
        self.fc2 = nn.Linear(hidden_dim_1 + time_emb_dim + sex_emb_dim + height_emb_dim, hidden_dim_2)
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, sex: torch.Tensor, height: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x (torch.Tensor): Noisy data of shape [batch_size, data_dim].
            t (torch.Tensor): Timestep tensor of shape [batch_size].
            sex (torch.Tensor): Binary sex tensor of shape [batch_size] or [batch_size, 1] (0 for female, 1 for male).
            height (torch.Tensor): Continuous height tensor of shape [batch_size] or [batch_size, 1].
        ...
        # Ensure sex is of shape [batch_size]. For nn.Embedding, we need a long tensor.
        if sex.ndim == 1:
            sex = sex.long()
        else:
            sex = sex.long().squeeze(1)
        sex_emb = self.sex_emb(sex)  # [batch_size, sex_emb_dim]
        
        # Ensure height is of shape [batch_size, 1].
        if height.ndim == 1:
            height = height.unsqueeze(1)
        height_emb = self.height_proj(height)  # [batch_size, height_emb_dim]
        
        # Concatenate the noisy data, time embedding, sex embedding, and height embedding.
        x_in = torch.cat([x, t_emb, sex_emb, height_emb], dim=1)  # [batch_size, input_dim]
        ...
        # Inject time, sex, and height embeddings again into the intermediate layer.
        inj = torch.cat([t_emb, sex_emb, height_emb], dim=1)  # [batch_size, (time_emb_dim+sex_emb_dim+height_emb_dim)]
        h1_inj = torch.cat([h1, inj], dim=1)  # [batch_size, hidden_dim_1 + (time_emb_dim+sex_emb_dim+height_emb_dim)]
        h2 = self.fc2(h1_inj)
        h2 = self.norm2(h2)
...
class DDPM(nn.Module):
    """The forward diffusion and backward denoising process for DDPM,
       with conditioning on sex and height."""
    ...
    def loss(self, x0: torch.Tensor, sex: torch.Tensor, height: torch.Tensor) -> torch.Tensor:
        """
        ...
        Args:
            ...
            sex (torch.Tensor): Sex conditioning tensor (binary; shape [batch_size] or [batch_size, 1]).
            height (torch.Tensor): Height conditioning tensor (continuous; shape [batch_size] or [batch_size, 1]).
        ...
        predicted_noise = self._noise_predictor(xt, t, sex, height)         # Predict noise using sex and height conditioning
        loss = torch.mean((predicted_noise - eps) ** 2)
        return loss

    def one_reverse_step(self, xt: torch.Tensor, t: int, sex: torch.Tensor, height: torch.Tensor) -> torch.Tensor:
        """
        ...
        and the conditioning variables sex and height.
        
        Args:
            xt (torch.Tensor): The current noisy input of shape [batch_size, snp_dim].
            t (int): The current timestep (should be >= 1).
            sex (torch.Tensor): Sex conditioning tensor.
            height (torch.Tensor): Height conditioning tensor.
        ...
        # Predict the noise using the conditioned noise predictor.
        eps_pred = self._noise_predictor(xt, t_tensor, sex, height)
    ...
    def sample(self, sample_size: int, sex: torch.Tensor, height: torch.Tensor) -> torch.Tensor:
        """
        ...
        """
        with torch.no_grad():
            x = torch.randn(sample_size, self._snp_dim, device=sex.device)
            # Iterate from the maximum timestep down to 1.
            for t in range(self._diffusion.tmax, 0, -1):
                x = self.one_reverse_step(x, t, sex, height)
        return x