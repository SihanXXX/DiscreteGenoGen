...
def train_evaluator(real, fake, metrics, label1, label2, save_path_img):
    """
    ...
    Returns:
        dict: A dictionary of computed metric values and we will also save the visualization images in the coreesponding path.
    """
    ...
    for metric_name in metrics:
        if metric_name in available_metrics:
            if metric_name == "pca":
                ...
                precision, recall = get_precision_recall(torch.tensor(real.to_numpy()).to(dtype=torch.float32), torch.tensor(fake.to_numpy()).to(dtype=torch.float32), ks=[60], distance = "euclidean")
